...
void Coin::Update(int state,int time,int judge)
{
	rad += 5;
	//チュートリアル
	if (state == STATE_CHUT)
...
/// </summary>
/// <param name="state">現在のステート</param>
void Coin::Draw(int state)
{
	if (Visible == false)
	{
		MV1DrawModel(Handle);
		DrawSphere3D(pos, coinradius, 100, (255, 255, 255), (255, 255, 255), false);
		if (state == STATE_CHUT)
		{
			Visible == true;
		}
