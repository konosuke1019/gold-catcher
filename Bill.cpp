...
void Bill::Update(int state, int time, int judge)
{
	rad += 5;
	//チュートリアル
	if (state == STATE_CHUT)
	{
		...
		MV1SetPosition(Handle, VGet(-0.2, pos.y, -0.2));
...
/// </summary>
/// <param name="state">現在のステート</param>
void Bill::Draw(int state)
{
	if (Visible == false)
	{
		MV1DrawModel(Handle);
		DrawSphere3D(pos, billradius, 100, (255, 255, 255), (255, 255, 255), false);
		if (state == STATE_CHUT)
		{
			Visible == true;
		}
